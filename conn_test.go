...
func TestConnHighGoDb(t *testing.T) {

	db, err := sql.Open("postgres", "postgres://syssso:Hello@123@192.168.70.105:5866/highgo")
	if err != nil {
	...
	err = rows.Scan(scans...)
	if err != nil {
		log.Fatalf("column error:%+v", err)
	}
	for k, v := range values {
		key := cols[k]
		row[key] = string(v)
	}
	fmt.Printf("res->%+v", row)
}
